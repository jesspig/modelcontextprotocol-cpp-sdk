...
JsonValue SerializeServerCapabilities(const ServerCapabilities& v) {
    JsonValue obj(JsonValue::object_tag);
    if (v.tools) obj[detail::kTools] = SerializeToolsCapability(*v.tools);
    if (v.resources) obj[detail::kResources] = SerializeResourcesCapability(*v.resources);
    if (v.prompts) obj[detail::kPrompts] = SerializePromptsCapability(*v.prompts);
...
ServerCapabilities DeserializeServerCapabilities(const JsonValue& j) {
    ...
    auto* s = j.Find(detail::kSampling);
    if (s && s->IsObject()) v.sampling = DeserializeSamplingCapability(*s);
    auto* rt = j.Find(detail::kRoots);
