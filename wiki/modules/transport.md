...
- POSIX 子进程：**argv/envp 构造与 PATH 搜索（`access` X_OK，仅自定义环境分支）全部在 `fork()` 之前完成**（fork 后仅 `chdir`/`dup2`/`execve` 等 async-signal-safe 调用），stderr 继承父进程；管道 fd 与标准流 dup 均设 `FD_CLOEXEC`（`SetCloseOnExec`）；析构 SIGKILL + WNOHANG reap
...
- **Windows `min` 宏与 GDI 防御**：`NOMINMAX` + `NOGDI` 经 `cmake/CompilerOptions.cmake` 在 clang-cl/MSVC 两分支全局定义（`NOGDI` 排除 `wingdi.h` 的 `GetObject` 宏，`NOMINMAX` 排除 `min`/`max` 宏）；传输库仍统一以 `(std::min)` 括号形式调用（SseClientTransport、HttpClient、Sha1 等）作纵深防御
