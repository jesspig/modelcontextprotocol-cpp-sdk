...
| EventStore | SSE 事件存储与断线回放（虚接口，内存实现为基类默认） | [/classes/file-event-store.md](../classes/file-event-store.md) |
| FileEventStore | EventStore 的 JSONL 文件持久化实现（跨进程文件锁） | [/classes/file-event-store.md](../classes/file-event-store.md) |
| SessionStore | 会话记录持久化抽象（stateful 模式外部会话接管） | [/concepts/storage.md](../concepts/storage.md) |
| StreamableHttpServerTransport | Streamable HTTP 服务端传输（Bearer 鉴权 / 会话接管） | [/transports/streamable-http.md](../transports/streamable-http.md) |
