...
- 无参构造；`Start()` 幂等（`running_.exchange(true)`），`OpenStandardInput/Output()` 打开管道，启动读线程——**由 `McpServer` 构造自动调用**（`ITransport::Start` 契约，[McpServer.cpp:219](../../src/server/McpServer.cpp)），服务端用户无需手动 Start
