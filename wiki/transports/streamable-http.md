...
- `SendMessageAsync`：stateless 先查 `pending_responses_` 完成 promise 关联；正常路径 `event: message` + 非 stateless 时 Append 到 EventStore 并带 `id:` 前缀，最后 `BroadcastSse`
...
- **平台双实现**：Win32 用 WinHTTP（`#pragma comment(lib, "winhttp.lib")`），POSIX 用 libhv `requests::request`；发送路径共用 `send_thread_ + send_queue_ + condition_variable`
