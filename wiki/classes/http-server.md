...
- handler 抛出的**任何 `std::exception`（含 McpError）→ 500**（[HttpServerImpl.cpp:452](../../src/http/HttpServerImpl.cpp)）；传输层（`StreamableHttpServerTransport`）在内部消化错误并映射 400/404，`McpError` 不会逃逸到本层
