...
- 入站校验用轻量视图：只构造 `{method, initialize 的 params, params._meta}`（`req.meta` 存在时置于视图 params 内，与线上一致）交给 codec，不完整序列化请求（[McpSessionHandler.cpp:232](../../src/protocol/McpSessionHandler.cpp)）
...
- `SendRequest` 双 era meta：modern era 把 `SerializeRequestMeta(meta)` 信封写入 `req.meta`（序列化层落 `params._meta`）；legacy era 仅当带 progressToken 时写 `params._meta.progressToken`（[McpSessionHandler.cpp:510](../../src/protocol/McpSessionHandler.cpp)）
...
- 订阅：`AddSubscription/RemoveSubscription/NotifySubscribers`，按 `SubscriptionFilter` 过滤，通知带 `subscriptionId` meta——事件通知的 `subscriptionId` **优先回显条目 `session_id`**（订阅时 `_meta` 携带的客户端 ID，与 ack 帧一致），未设置时回退服务端自增 `id`（[McpSessionHandler.cpp:685](../../src/protocol/McpSessionHandler.cpp)）
