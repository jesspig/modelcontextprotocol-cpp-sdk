...
Auto 回退分派（对齐官方 TS SDK，[McpClient.cpp:261](../../src/client/McpClient.cpp)）：
...
- **token 生成**：`options.meta.progressToken`（string/int）显式提供时优先透传，否则 `next_progress_token_` 原子计数自 1 起自动生成（`AttachProgressCallback`，[McpClient.cpp:669](../../src/client/McpClient.cpp)）
...
- **双 era 落点**：legacy era progressToken 写 `params._meta.progressToken`（[McpSessionHandler.cpp:499](../../src/protocol/McpSessionHandler.cpp)）；modern era 走 `_meta` 信封、序列化层落 `params._meta`

## 客户端 → 服务端通知

`SendRootsListChanged()` 发送 `notifications/roots/list_changed`（[McpClient.cpp:1226](../../src/client/McpClient.cpp)）。协商版本须 **>= 2025-06-18**（在 `kProtocolVersions` 中比较序位），否则抛 `McpError(ProtocolViolation)`。
