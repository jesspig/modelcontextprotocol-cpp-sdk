...
- `Dump(int indent = -1)` 手写序列化（[JsonValue.cpp](../../src/core/JsonValue.cpp)）：控制字符 `\uXXXX` 转义；double 用 `max_digits10`，经 `std::to_chars` 输出（**locale 无关**，MSVC/glibc≥11 或 `__cpp_lib_to_chars` 启用，否则回退 `snprintf("%.17g")`），NaN/Inf 输出 `null`；无 `.eE` 时补 `.0`
