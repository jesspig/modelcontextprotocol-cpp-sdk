...
- `Parse` 用 simdjson DOM 递归转换，失败抛 `McpError(ParseError)`；uint64 超 int64 范围抛 `DeserializeFailed`
- 8 个类型检查 + 7 组 Get* 访问器（类型不匹配抛 `DeserializeFailed`）
- `operator[]`：非 const 缺失键**插入 null**；const 缺失抛异常；`Find` 缺失返回 nullptr；`At` 缺失抛异常
