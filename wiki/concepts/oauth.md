...
解析 `resource_metadata="<uri>"` → 拉取资源元数据合并进 metadata_，随后 `VerifyResourceMatch` **事后校验**（[OAuthClientProvider.cpp:121](../../src/client/auth/OAuthClientProvider.cpp)）：`OAuthMetadata.resource` 字段须与 `options_.server_url` **精确相等**或匹配其 **scheme+host** 前缀；字段缺失/空则放行；不匹配抛 `McpError(InternalError)`。
