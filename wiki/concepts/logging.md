...
- `mcp::SetLogHandler(LogHandler)`：设置自定义回调；传 `nullptr` 恢复默认 `stderr` 输出。回调在发射线程同步执行，异常由日志层吞掉并记录为 Error，不能把抛出异常作为业务控制流。
...
- **Span 观测钩子**：协议层另有 `SpanHandler`，覆盖请求和传输边界，接入规则见 [/concepts/span-hooks.md](../concepts/span-hooks.md)
