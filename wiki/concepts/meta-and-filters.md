...
- **2026 时代缓存字段平铺**：`WireCodec::EncodeResult` 将结果中嵌套的 `cacheHint` 对象转为**顶层** `ttlMs`/`cacheScope` 后删除 `cacheHint`（[WireCodec.cpp](../../src/protocol/WireCodec.cpp:203)）；2025 时代保留嵌套原样
